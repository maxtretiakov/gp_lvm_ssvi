...
def optimize_latents(inner_iters, opt_x, mu_x, log_s2x, Y, K_inv, noise_var, m_u, C_u, Sigma_det, idx, Z, DEV,log_sf2, log_alpha, NUM_U_SAMPLES, GR_CLIP):
    ...
    for _ in range(inner_iters):
        ...
        elbo_vec = vmap(lambda u: local_step(idx=idx, U_sample=u, Sigma_det=Sigma_det, update_beta=False, mu_x=mu_x, log_s2x=log_s2x, Y=Y, K_inv=K_inv, noise_var=noise_var, log_sf2=log_sf2, log_alpha=log_alpha, Z=Z, DEV=DEV)[0])(U_smpls)
...
def local_step(idx, U_sample, Sigma_det, update_beta, 
               mu_x, log_s2x, Y, K_inv, noise_var, log_sf2, log_alpha, Z, DEV,
               dbg=False):
    """
    ...
    psi0, psi1, psi2 = compute_psi(mu=mu, s2=s2, log_sf2=log_sf2, log_alpha=log_alpha, Z=Z, DEV=DEV)  # (B,), (B,M), (B,M,M)
    A = psi1 @ K_inv  # (B, M)
